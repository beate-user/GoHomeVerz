...
*** diffs between branches without merge commits or cherry picks
git log --cherry-pick --right-only origin/master...origin/release/5.4.1 --no-merges
...
* Version markieren / tagging
git tag v1.0
Taggt aktuelles Commit als v1.0
** rename
git tag new old
git tag -d old
git push origin new :old       # Push `new` to your remote named "origin", and delete tag `old`.
** signing
 ...
 * git checkout --ours codefile.js
   selects changes from *[[master1]]*
 * git checkout --theirs codefile.js
   selects changes from *[[feature1]]*
*** rebase
 git checkout <<feature2>>
 git rebase <<master2>>
 * git checkout --ours codefile.js
   selects changes from *[[master2]]*
 * git checkout --theirs codefile.js
   selects changes from *[[feature2]]*
*** stash
 git checkout <<master3>>
 git <<stash>> pop
 * git checkout --ours codefile.js
   selects changes from *[[master3]]*
 * git checkout --theirs codefile.js
   selects changes from *[[stash]]*
** commit reference
*** merge / MERGE_HEAD
 * git restore *HEAD* thing.js
   use what the *current* branch has
 * git restore MERGE_HEAD thing.js
   use what the other branch has
*** rebase / REBASE_HEAD
 * git restore *HEAD* thing.js
   use what the *current* branch has
 * git restore REBASE_HEAD thing.js
   use what the other branch has
*** cherry pick / CHERRY_PICK_HEAD
 * git restore *HEAD* thing.js
   use what the *current* branch has
 * git restore CHERRY_PICK_HEAD thing.js
   use what the other branch has
* Stashing
