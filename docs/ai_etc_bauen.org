...
** ssh
*** ssh pub key login
**** extract pub key from priv key
put into auth_keys on remote host
ssh-keygen -f /tmp/pke.txt -y
**** get serverkey
put into known_keys
ssh-keyscan 10.11.48.37
**** extract short serverkey
you can find it if ssh -vvv is called
ssh-keygen -l -F 10.11.48.37
*** ssh-agent
eval `ssh-agent -s`
...
** arch linux
*** update all
pacman-key --refresh-keys && pacman -S archlinux-keyring --noconfirm && pacman -Suy --noconfirm
